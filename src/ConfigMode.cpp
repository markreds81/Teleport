...
void ZConfig::doModeCommand()
{
    ...
    case ZCFGMENU_ADDRESS:
    {
        ...
        if (cmd.equalsIgnoreCase("delete") && (entry != nullptr))
        {
            delete entry;
            currState = ZCFGMENU_MAIN;
            serial.printf("%sPhonebook entry deleted.%s%s", EOLNC, EOLNC, EOLNC);
        }
        else if ((cmd.length() == 0) && (entry != nullptr))
    ...
    case ZCFGMENU_NETMENU:
    {
        ...
        {
            currState = ZCFGMENU_MAIN;
            if ((staticIP == nullptr) && (useDHCP))
            {
            }
            else if (useDHCP)
            {
                setNewStaticIPs(nullptr, nullptr, nullptr, nullptr);
                if (!connectWifi(wifiSSI.c_str(), wifiPWD.c_str(), staticIP, staticDNS, staticGW, staticSN))
                    serial.printf("%sUnable to connect to %s. :(%s", EOLNC, wifiSSI.c_str(), EOLNC);
                settingsChanged = true;
            }
            else if ((staticIP == nullptr) || (*staticIP != lastIP) || (*staticDNS != lastDNS) || (*staticGW != lastGW) || (*staticSN != lastSN))
            {
    ...
    case ZCFGMENU_WIMENU:
    {
        ...
        else
        {
            ...
            {
                if (!connectWifi(WiFi.SSID(num - 1).c_str(), "", nullptr, nullptr, nullptr, nullptr))
                {
    ...
    case ZCFGMENU_WIFIPW:
        ...
        else
        {
            ...
            if (!connectWifi(WiFi.SSID(lastNumber).c_str(), cmd.c_str(), nullptr, nullptr, nullptr, nullptr))
                serial.printf("%sUnable to connect to %s.%s", EOLNC, WiFi.SSID(lastNumber).c_str(), EOLNC);
            else
            {
                //setNewStaticIPs(null,null,null,null);
                useDHCP = (staticIP == nullptr);
                lastIP = (staticIP != nullptr) ? *staticIP : WiFi.localIP();
                lastDNS = (staticDNS != nullptr) ? *staticDNS : IPAddress(192, 168, 0, 1);
                lastGW = (staticGW != nullptr) ? *staticGW : IPAddress(192, 168, 0, 1);
                lastSN = (staticSN != nullptr) ? *staticSN : IPAddress(255, 255, 255, 0);
...
void ZConfig::loop()
{
    if (showMenu)
    {
        ...
        case ZCFGMENU_OPTIONS:
        {
            ...
            serial.printf("[FLOW] Control: %s%s", flags.xonxoff ? "XON/XOFF" : flags.rtscts ? "RTS/CTS"
                                                                                            : "DISABLED",
                          EOLNC);
