...
ZResult PrintHandler::switchToPostScript(char *prefix)
{
	...
	ZResult result;
	digitalWrite(PIN_RTS, rtsInactive);
	yield();
	...
	if ((result != ZERROR) && (prefix != 0) && (strlen(prefix) > 0))
	{
...
ZResult PrintHandler::switchTo(char *vbuf, int vlen, bool petscii)
{
	...
	if ((vlen <= 2) || (workBuf[1] != ':'))
	{
		...
		if ((vlen == 1) && (strchr("parPAR", workBuf[0]) != 0))
...
void PrintHandler::serialIncoming()
{
	if (SerialDTE.available() > 0)
	{
		while (SerialDTE.available() > 0)
		{
			...
			if (pdex >= 250)
			{
				if (((wifiSock != nullptr) && (wifiSock->isConnected())) || ((wifiSock == nullptr) && (outStream != nullptr)))
				{
...
void PrintHandler::loop()
{
	...
	{
		SerialDebug.println("No printer connection");
		switchBackToCommandMode(true);
	}
	else if (millis() > currentExpiresTimeMs)
	{
		SerialDebug.println("Time-out in printing");
		if (pdex > 0)
			writeChunk(pbuf, pdex);
		writeStr("0\r\n\r\n");
		outStream->flush();
