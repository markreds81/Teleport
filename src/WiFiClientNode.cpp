...
{
    return SSL ? new WiFiClientSecure() : new WiFiClient();
}
...
{
    return (host != nullptr) && client.connected();
}
...
{
    if ((host != nullptr) && (client.available() == 0))
        flushAlways();
}
...
int WiFiClientNode::getNumOpenWiFiConnections()
{
    ...
    while (conn != nullptr)
    {
        ...
        if ((chkConn->nextDisconnect != 0) && (millis() > chkConn->nextDisconnect))
            delete (chkConn);
        else if ((chkConn->isConnected() || (chkConn->available() > 0) || ((chkConn == conns) && ((serialOutBufferBytesRemaining() < SER_WRITE_BUFSIZE - 1) || (SerialDTE.availableForWrite() < SER_BUFSIZE)))) && chkConn->isAnswered())
            num++;
    }
    return num;
}
