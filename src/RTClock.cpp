...
RealTimeClock::RealTimeClock(int y, int m, int d, int h, int mn, int s, int mi) : DateTimeClock(y, m, d, h, mn, s, mi)
{
    lastMillis = millis();
    nextNTPMillis = millis();
}
...
void RealTimeClock::tick()
{
    ...
    if (udpStarted)
    {
        ...
        else
        {
            ...
            {
                if (((nextNTPMillis - now) > ntpPeriodLongMillis) || (nextNTPMillis == now))
                    forceUpdate();
            }
...
bool RealTimeClock::sendTimeRequest()
{
    ...
    {
        // adapted from code by  by Michael Margolis, Tom Igoe, and Ivan Grokhotkov
        SerialDebug.println("Sending NTP Packet...");
        byte packetBuffer[NTP_PACKET_SIZE];
